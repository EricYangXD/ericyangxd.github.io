...
- ![原型关系图](https://cdn.jsdelivr.net/gh/EricYangXD/vital-images@master/imgs/proto.png "原型关系图")

ECMAScript 中**所有函数的参数都是按值传递的**。也就是说，把函数外部的值复制给函数内部的参数，就和把值从一个变量复制到另一个变量一样。
...
面试时问到原型的回答，要围绕原型、原型链、继承、原型的工作原理及相关概念回答。

1. JavaScript 是一种基于原型的面向对象语言，原型是对象之间继承和共享属性和方法的机制。
2. 在 JavaScript 中，每个实例对象都有一个内部属性 `[[Prototype]]`（可以通过 `__proto__` 访问），指向其原型对象--Parent.prototype。原型对象可以包含属性和方法，子对象可以通过原型链访问这些属性和方法。
3. 当使用构造函数创建对象时，构造函数的 prototype 属性指向原型对象。所有通过该构造函数创建的实例都将共享 prototype 上的属性和方法。
4. 当尝试访问对象的属性或方法时，JavaScript 引擎首先会查找该对象本身，如果找不到，则会查找其原型对象，如果仍然找不到，就会查找原型的原型，一直到`Object.prototype => null` 为止，这个过程称为原型链。
5. 原型是动态的，可以在运行时添加或修改属性和方法。这意味着，所有已经创建的实例在原型上添加的新属性或方法将会被所有实例访问到。
6. JavaScript 中的原型机制也支持继承。通过设置子类的原型为父类的一个实例，可以实现继承。原型链继承：一种是在子类中调用父类的构造函数`Parent.call(this, xx)`，或者通过 `Son.prototype = Object.create(Parent.prototype)` 。最后记得设置子类的 constructor 为子类的构造函数。
...
- 真值表如下:
- ![JS类型转换真值表](https://cdn.jsdelivr.net/gh/EricYangXD/vital-images@master/imgs/js-true-value.png "JS类型转换真值表")

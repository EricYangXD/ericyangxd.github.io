...
5. 隐式网格属性`grid-auto-rows`：结合父级设置`white-space: nowrap;`强制不换行，让多行长度不一的区块自动排列，在纵向上实现现跨天行号一致，比如日历中的跨天活动，优化展示效果
...
- grid 中 auto-fit 和 auto-fill 的区别:
  ...
  - auto-fill: 「尽可能多地填充列，即使没有内容也会 “占位”」，空轨道保留，不拉伸内容
  - auto-fit: 「自动适应内容，能合并多余空列，不占位」，空轨道折叠，拉伸内容
